...
def generate_synthetic_disaster_data(n_samples: int = 500) -> np.ndarray:
    ...
    for t in range(n_samples):
        ...
        base_rainfall = np.random.rand(GRID_HEIGHT, GRID_WIDTH)
        ...
        monsoon_factor = 0.5 * np.sin(2 * np.pi * (month - 3) / 12) + 0.5
        data[t, :, :, 0] = np.clip(base_rainfall * monsoon_factor, 0, 1)

        # --- Temperature channel (hotter in summer) ---
        base_temp = np.random.rand(GRID_HEIGHT, GRID_WIDTH)
        # Northern plains are hotter in summer
        base_temp[:10, :] += 0.2
        temp_factor = 0.5 * np.sin(2 * np.pi * (month - 1) / 12) + 0.5
        data[t, :, :, 1] = np.clip(base_temp * temp_factor, 0, 1)

        # --- Disaster index (correlated with rainfall and temperature) ---
        disaster = (
            0.5 * data[t, :, :, 0]          # floods follow rainfall
            + 0.3 * data[t, :, :, 1]        # heat-wave index
            + 0.2 * np.random.rand(GRID_HEIGHT, GRID_WIDTH)
        )
        # Coastal areas (bottom rows) hit by cyclones during Oct–Nov
        if month in (9, 10):
            disaster[-3:, :] += 0.4
        data[t, :, :, 2] = np.clip(disaster, 0, 1)
...
def evaluate_model(
    ...
    """
    Evaluate the model on the test set and return regression metrics.
